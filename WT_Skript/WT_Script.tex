...
\section{Gemeinsame Verteilung}
	...
	Ein diskreter W-Raum $(\Omega,\mathcal{P}(\Omega),P)$.\\
	...
	%Needs to be alignment --> linke Seite is extra alignment what gets reffered too	
	\begin{itemize}
		\item rechte Seite: $A\subset\Omega_X\rightarrow P(X\in A) \tilde{=}$ Verteilung von X\\
		$B\subset\Omega_Y\rightarrow P(Y\in B) \tilde{=}$ Verteilung von Y
		\item linke Seite: $(A\in\Omega_X , B\subset\Omega_Y)\rightarrow P(X\in A, Y\in B)$(*)
	...
	\paragraph{Frage:}
	Stellt die Abbildung (*) ebenfalls eine Verteilung/W-Maß dar?
	\paragraph{Antwort:}
	Das ist nicht der Fall, da der Definitionsbereich der Abbildung (*) zu klein ist. Er lässt sich nur mit einer echten Teilmenge der Potenzmenge des karthesischen Produktes $\Omega_X \times\Omega_Y$ identifizieren.
	\paragraph{Beispiel:}
	...
	\paragraph{Diskussion:} minimale Forderung an die Erweiterung des Definitionsbereiches(aus (*)). \\
	...
	\paragraph{Bemerkung:}
	Man kann zeigen, dass es eine eindeutige Fortsetzung der Abbildung (*) zu einem W-Maß auf $\mathcal{P}(\Omega_X \times \Omega_Y)$ gibt und diese dann mit der gemeinsamen Verteilung von $X$ und $Y$ übereinstimmt.
	...
	$$X \text{ : Augenzahl bei i Versuchen}$$
