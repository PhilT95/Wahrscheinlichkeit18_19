...
= np(1-p)
	...
	\begin{itemize}
	\item $$x\in\Omega\longmapsto\rho_X (x):=P(X=x) \text{ ist die Gewichtsfunktion der Verteilung } P_X \text{ von } X$$
	\item $$y\in\Omega_Y\longmapsto\rho_Y(y):=P(Y=y) \text{ ist die Gewichtsfunktion der Verteilung } P_Y \text{ von } Y$$
	\item $$(x,y)\in\Omega_X \times \Omega_Y\longmapsto P(X=x,Y=y) \text{ ist die Gewichtsfunktion der Verteilung einer Zufallsvariable mit Werten aus } \Omega_X \times \Omega_Y$$
	\end{itemize}
	...
	\end{equation*}	 	
 	...
 	\begin{itemize}
 	\item[Gegeben:] $A,B\subseteq\Omega$ zwei Ereignisse
 	\item[Annahme:] $P(A)>0$
 	\end{itemize}
